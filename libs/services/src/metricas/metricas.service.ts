...
export class MetricasService {
    ...
    async GetMetricas(Year?: number) {
        if (Year) {
            const fechaFromYear = new Date(moment().set('day', 1).set('month', 1).set('year', Year).set('hours', 0).set('minutes', 0).set('seconds', 0).set('milliseconds', 0).utcOffset(0, true).format())
            let MetricasYear = await this.metricasRepository.find({  where: { created_at: MoreThanOrEqual(fechaFromYear) } })
            return (MetricasYear)
        } else {
    ...
    async configMetricas() {
        ...
        const metrica:any = await this.GetMetricas()
